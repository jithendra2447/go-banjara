...
export default function DiscountSalePage() {
  ...
  const filteredProducts = React.useMemo(() => {
    // Basic filter: only show items with discounts
    let list = productsList.filter(
      (prod) => prod.originalPrice && prod.originalPrice > prod.price
    );
    ...
    <div className="bg-white min-h-screen pb-0 flex flex-col items-center">
      ...
      <header className="max-w-[1440px] w-full mx-auto text-left space-y-6 px-6 md:px-[80px] pt-[62px] pb-[24px]">
        ...
        <h1 className="text-3xl sm:text-4xl md:text-[44px] font-serif text-[#1A1A1A] tracking-tight leading-[1.15] max-w-none font-medium">
          Shop Products in Discount <span className="text-[#FF5B37]">Sale</span>
        </h1>
